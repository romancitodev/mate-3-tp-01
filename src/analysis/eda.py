...
def analyze_target(df: pd.DataFrame):    
    def graficar(df: pd.DataFrame):
        ...
        sns.histplot(data=df, x='alcohol', bins=30, color="#9F6BC2", edgecolor='black', alpha=0.7, ax=axes[1, 0]).set_title("Distribucion del contenido de alcohol")

        # Relación alcohol vs calidad
        quality_groups = df.groupby('quality')['alcohol'].mean()
        sns.lineplot(x=quality_groups.index, y=quality_groups.values, marker='o', linewidth=2, markersize=8, ax=axes[1, 1],color="#9F6BC2").set_title("Alcohol Promedio por Calidad")
